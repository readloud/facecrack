 function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }import yargs from "yargs";
import fs from "fs";
...
const main = async () => {
	const target = _nullishCoalesce(opt.t, () => ( opt.target));
	const wordlist = _nullishCoalesce(_nullishCoalesce(opt.w, () => ( opt.wl)), () => ( opt.wordlist));
	const executable = _nullishCoalesce(opt.exec, () => ( opt.executable));
	const delayTime = _nullishCoalesce(opt.d, () => ( Number(opt.delay)));
